# 국토교통부 단독/다가구 전월세 실거래가 API 명세

> **Version:** 1.1.0 (2026-02-09)
> **Base URL:** `https://apis.data.go.kr/1613000/RTMSDataSvcSHRent`

---

## 개요

「부동산 거래신고 등에 관한 법률」에 따라 신고된 자료 및 「주택임대차보호법」에 따라 부여된 **확정일자 자료**로서, 행정표준코드관리시스템(www.code.go.kr)의 법정동 코드 중 앞 5자리와 계약년월 6자리로 해당 지역, 해당 기간의 단독/다가구주택의 전월세 신고정보를 조회할 수 있습니다.
...
```
GET https://apis.data.go.kr/1613000/RTMSDataSvcSHRent/getRTMSDataSvcSHRent
    ?LAWD_CD=11110
    &DEAL_YMD=202501
    &serviceKey={인증키}
```
...
| `preDeposit` | string | 종전 보증금 (갱신 시) | `"9,500"` |
...
- 다가구주택의 경우 건물 전체 연면적 제공
- 실제 임대 면적과 다를 수 있음

> **참고:** 아파트/연립다세대는 `excluUseAr`(전용면적)을 제공하지만,
> 단독/다가구는 `totalFloorAr`(연면적)을 제공합니다.
...
| 항목 | 단독/다가구 매매 | 단독/다가구 전월세 |
|------|-----------------|-------------------|
| **Base URL** | RTMSDataSvcSHTrade | RTMSDataSvcSHRent |
| **가격 필드** | `dealAmount` | `deposit`, `monthlyRent` |
| **주택유형** | `houseType` | 없음 |
| **지번** | `jibun` (일부) | 없음 |
| **대지면적** | `plottageAr` | 없음 |
| **연면적** | `totalFloorAr` | `totalFloorAr` |
...
| 항목 | 아파트 전월세 | 연립다세대 전월세 | 단독/다가구 전월세 |
|------|--------------|-------------------|-------------------|
| **Base URL** | RTMSDataSvcAptRent | RTMSDataSvcRHRent | RTMSDataSvcSHRent |
...
### 단독/다가구 전월세 특징
- **건물명 없음**: 개별 주택이므로 지역+동으로만 식별
- **지번 없음**: 개인정보보호로 전월세에서는 미제공
- **층 정보 없음**: 단독주택이므로 특정 층 개념 없음
- **연면적 제공**: 전용면적 대신 건물 전체 면적
...
| 데이터포맷 | XML |
| 참고문서 | 단독다가구 전월세 실거래가 자료 기술문서.hwp |

### 상세기능정보

| NO | 상세기능 | 설명 | 일일 트래픽 |
|----|----------|------|-------------|
| 1 | 단독/다가구 전월세 실거래가 공개 자료 `/getRTMSDataSvcSHRent` | 부동산 거래신고에 관한 법률에 따라 신고된 거래의 실거래 자료를 제공 | 10,000 |
...
- **참고문서:** 단독다가구 전월세 실거래가 자료 기술문서.hwp (공공데이터포털 다운로드)
